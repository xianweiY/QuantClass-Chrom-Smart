...
def _build_agent_messages(agent: Dict, ctx_block: str, question: str, language: str):
    ...
    If the agent has a ``skill_id``, the skill's thinking framework is
    prepended so the agent answers through that person's perspective.
    """
    messages = []

    # Inject linked skill (thinking framework) if present
    skill_id = agent.get("skill_id", "")
    if skill_id:
        from services.skill_service import get_skill_prompt, get_skill
        skill_prompt = get_skill_prompt(skill_id)
        if skill_prompt:
            skill_info = get_skill(skill_id)
            skill_name = (skill_info or {}).get("display_name", skill_id)
            messages.append(LLMMessage(
                role="user",
                content=(
                    f"你现在是「{skill_name}」。以下是你的完整思维操作系统，"
                    f"你必须完全按照这个框架来思考和回答。"
                    f"体现这个人物的核心心智模型、决策启发式和表达风格，"
                    f"以第一人称「我」来回答。\n\n{skill_prompt}"
                ),
            ))
            messages.append(LLMMessage(
                role="assistant",
                content=f"好的，我现在是{skill_name}，会用我的思维框架来分析。",
            ))
...
async def discuss(
    ...
    async def _call_agent(agent: Dict) -> Dict[str, Any]:
        ...
        # Resolve skill display info for the frontend
        skill_info = {}
        if agent.get("skill_id"):
            from services.skill_service import get_skill
            sk = get_skill(agent["skill_id"])
            if sk:
                skill_info = {"skill_id": sk["id"], "skill_name": sk.get("display_name") or sk["name"], "skill_icon": sk.get("icon", "🧠"), "skill_icon_url": sk.get("icon_url")}
        try:
